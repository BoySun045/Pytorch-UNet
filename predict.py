...
from utils.pred_to_line import df_to_linemap, df_wf_to_linemap, df_cls_to_line_weight
...
def resize_crop_img(img, scale, img_size):
    # Resize the image
...
def create_pred_vis_overlay(line_map, img, scale, non_interactive=False):
...
#             line_mask, line_map = df_cls_to_line_weight(df=df_pred, cls_mask=label_mask_pred, 
#                                                         df_neighborhood=10, threshold=0.1,
#                                                         bin_edges=bin_edges)
